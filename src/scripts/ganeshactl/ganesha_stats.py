...
commands = ('help', 'list_clients', 'deleg', 'global', 'inode', 'iov3', 'iov4',
	    ...
	    'disable', 'status', 'pool', 'v3_full', 'v4_full')
...
# requires fsal name
    ...
    if not len(sys.argv) == 3:
	print "Option \"%s\" must be followed by all/nfs/fsal/v3_full/v4_full/rpc." % (command)
	usage()
    command_arg = sys.argv[2]
    if command_arg not in ('all', 'nfs', 'fsal', 'v3_full', 'v4_full', 'rpc'):
	print "Option \"%s\" must be followed by all/nfs/fsal/v3_full/v4_full/rpc." % (command)
	usage()
